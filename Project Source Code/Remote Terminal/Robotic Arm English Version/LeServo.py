#!! /usr/bin/python3
# encoding: utf-8

import pigpio 
import time    
import threading 

class PWM_Servo(object): 

    def __init__(self, pi, pin, freq = 50, min_width = 500, max_width=2500, deviation=0, control_speed=False):
        self.pi = pi  
        self.SPin = pin   # Servo motor pin
        self.Freq = freq 
        self.Min = min_width   
        self.Max = max_width  
        self.Deviation = deviation 
        self.speedControl = control_speed 
        
        self.Position = 1500  
        self.positionSet = self.Position 
        
        self.stepTime = 20  # Move time for each section
        self.positionInc = 0.0 # Position difference
        self.Time = 0 
        self.Time_t = 0 
        self.incTimes = 0  # The number of sections
        
        self.positionSet_t = 0 # Angle
        self.posChanged = False  
        self.servoRunning = False  

        #self.pi.set_PWM_range(self.SPin, 1000000 / self.Freq) 
        #self.pi.set_PWM_frequency(self.SPin, self.Freq) 
        
        if control_speed is True: # Speed changes
            t = threading.Thread(target=PWM_Servo.updatePosition, args=(self,))
            t.setDaemon(True)
            t.start()

    def setPosition(self, pos, time = 0): # Default zero
        if pos < self.Min or pos > self.Max: # Aviod exceeding the range
            print(pos)
            return
        if time == 0: 
            self.Position = pos
            self.positionSet = self.Position
            self.pi.set_PWM_dutycycle(self.SPin, self.Position + self.Deviation) 
        else:
        # Time limitation for each section 
            if time < 20: # Min
                self.Time_t = 20
            elif time > 30000: # Max
                self.Time_t = 30000
            else:
                self.Time_t = time
            self.positionSet_t = pos 
            self.posChanged = True 

            
    def getPosition(self): # Current angle
        return self.Position

    def updatePosition(self):
       
        while True:
            if self.posChanged is True: 
                self.Time = self.Time_t  
                self.positionSet = self.positionSet_t 
                self.posChanged = False

                self.incTimes = int(self.Time / self.stepTime )
                self.positionInc =  self.Position - self.positionSet 
                self.positionInc = int(self.positionInc / self.incTimes) 
                self.servoRunning = True 

            if self.servoRunning is True:
                self.incTimes -= 1 
                if self.incTimes == 0: 
                    self.Position = self.positionSet 
                    self.servoRunning = False 
                else:
                    self.Position = self.positionSet + self.positionInc * self.incTimes 
                #self.pi.set_PWM_dutycycle(self.SPin, self.Position + self.Deviation) 
                self.pi.set_servo_pulsewidth(self.SPin, int(self.Position + self.Deviation) )
            time.sleep(0.02) 
            
            
        def setDeviation(newD = 0):
            if newD > 300 or newD < -300: 
                return
            self.Deviation = newD






