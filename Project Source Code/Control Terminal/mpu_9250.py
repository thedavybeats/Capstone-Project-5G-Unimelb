...
class MPU9250:
    ...
    magScale = [1.3116076970825574, 0.817092034029389, 0.9864612511671336] # NEED CHANGE ******** SID 
    ...
    mbias = [2.1085164835164836, -11.00235615079365, -1.8554945054945053] # Magnetometer Hard Iron Distortion ******** HID
...
#             if self.address_mpu_master is 0x69:
...
#         print(packet_count)
    ...
    def get_acc_angles(self):
        [ax, ay, az] = self.readAccelerometerMaster()
        phi = math.atan2(ay, math.sqrt(ax ** 2.0 + az ** 2.0))
        theta = math.atan2(-ax, math.sqrt(ay ** 2.0 + az ** 2.0))
        return [phi, theta]
    def compFilter(self,ax,ay,az,gx,gy,gz,preroll,prepitch):
        ...
        gyroRoll = preroll - gy * dt
        gyroPitch = prepitch + gx * dt
        self.gyroYaw += gz * dt
        #yaw = round(self.gyroYaw)
        yaw = 0
        # Comp filter
        roll = round((self.tau)*(preroll - gy*dt) + (1-self.tau)*(accRoll))
        pitch = round((self.tau)*(prepitch + gx*dt) + (1-self.tau)*(accPitch))
